...
QBean.openStream = function openStream( host, port, callback ) {
//console.log("AR: connecting to", host, port);
    ...
    this.getReply = function getReply( ) {
        ...
        if (hasBody) {
            ...
            if (reply.reply.slice(0, 3) === 'OK ' && reply.body.slice(0, 3) === '---') {
                ...
                try {
                    // TODO: parse the yaml string.  yaml.eval fails (formatting?)
                    reply.body = this.parseYaml(reply.body);
                }
                catch (err) { /* leave body as text */ }
            }
        }
        else {
...
//console.log("AR: sendCommand of", cmdline, dataline);
    this.conn.sendCommand(cmdline, dataline, function(err) {
        // FIXME: if (err) a send error occurred... how to repatriate if sends are batched?
    });
