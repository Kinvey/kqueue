...
function KQueue( options ) {
    ...
    this.beanWrapper = settings.beanWrapper;
    this.bean = settings.beanstalkClient;
...
KQueue.prototype.deleteJob = function deleteJob( job, cb ) {
    cb = cb || function(){ };
    this.bean.destroy(job.jobid, function(err) {
        if (err) return cb(KQueue.fixupError(err));
...
KQueue.prototype.ungetJob = function ungetJob( job, newPriority, afterDelaySeconds, cb ) {
    cb = cb || function(){ };
    this.bean.release(job.id, newPriority, afterDelaySeconds, function(err) {
...
KQueue.prototype.retryJob = function retryJob( job, reasonMessage, cb ) {
    ...
    this.bean.release(job.id, job.priority, this.config.retryDelaySec, function(err) {
        if (err) cb(KQueue.fixupError(err));
        else cb();
    });
};
...
// This will guarantee that the job was registered (and not silently dropped).
        ...
        function(err) {
            ...
            if (err) this.log.error("unable to insert (try again):", err);
    ...
    try {
        ...
        if (payload.length <= 65535) {
            ...
            this.jobStore.set(job._id, payload, function(err) {
                ...
                payload = JSON.stringify(job);
                self.bean.put(job.priority, job.delay, job.ttr, payload, function(err, ret) {
                    if (err) err = KQueue.fixupError(err);
                    cb(err, ret);
                });
            });
