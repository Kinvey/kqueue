...
module.exports.buildQueue = function buildQueue( config, callback ) {
    ...
    createStore(config, config.store || config, function(err, store) {
    ...
    function createStore( config, storeConfig, cb ) {
        if (config.bulkStore) {
            return cb(null, config.bulkStore);
        }
        else if (storeConfig.mongodbUrl) {
            var mongoConfig = {db: {safe: true, w: 1}, server: {poolSize: 1}};
            mongo.connect(storeConfig.mongodbUrl, mongoConfig, function(err, db) {
                var collection = db.db(storeConfig.dbname || 'kqueue').collection(storeConfig.collection || 'store');
